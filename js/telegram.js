...
async function loadTelegramSettings(){clearTimeout(telegramSaveTimer);const eventId=selectedEventId();if(!eventId){$('telegramStatusEnabled').checked=false;$('telegramDeadlineEnabled').checked=false;setResult('Seleziona una gara.');return}const {data,error}=await db.from('v2_event_config').select('key,value').eq('event_id',eventId).in('key',KEYS);if(error){setResult('Errore caricamento impostazioni Telegram: '+error.message,true);return}const map=Object.fromEntries((data||[]).map(r=>[r.key,r.value]));$('telegramStatusEnabled').checked=map.telegram_status_enabled===true;$('telegramDeadlineEnabled').checked=map.telegram_deadline_enabled===true;setResult('');}
async function saveTelegramSettings(snapshot){const eventId=snapshot?.eventId||selectedEventId();if(!eventId)return toast('Seleziona una gara');setResult('Salvataggio automatico…');const rows=[{event_id:eventId,key:'telegram_status_enabled',value:snapshot?.status??$('telegramStatusEnabled').checked,is_public:false},{event_id:eventId,key:'telegram_deadline_enabled',value:snapshot?.deadline??$('telegramDeadlineEnabled').checked,is_public:false}];const {error}=await db.from('v2_event_config').upsert(rows,{onConflict:'event_id,key'});if(error){setResult('Errore salvataggio: '+error.message,true);return toast(error.message)}if(selectedEventId()===eventId)setResult('Salvato ✓');}
function scheduleTelegramSave(){const eventId=selectedEventId();if(!eventId)return toast('Seleziona una gara');const snapshot={eventId,status:$('telegramStatusEnabled').checked,deadline:$('telegramDeadlineEnabled').checked};setResult('Modifiche non salvate');clearTimeout(telegramSaveTimer);telegramSaveTimer=setTimeout(()=>saveTelegramSettings(snapshot),350)}
...
$('telegramStatusEnabled')?.addEventListener('change',scheduleTelegramSave);
$('telegramDeadlineEnabled')?.addEventListener('change',scheduleTelegramSave);
