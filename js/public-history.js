...
function resultKey(athlete,event){return `${String(athlete||'').trim().toLocaleUpperCase('it')}||${String(event||'').trim().toLocaleUpperCase('it')}`}
  ...
  historyData.athletes.filter(a=>(!q||a.full_name.toLocaleLowerCase('it').includes(q))&&(cat==='all'||a.category===cat)).forEach(a=>{
    ...
    if(attended.length){
      ...
      attended.forEach(e=>{
        ...
        const rows=historyResults.get(resultKey(a.full_name,e.title))||[];
        let eventName;
        ...
        const days=document.createElement('span');days.textContent=eventDays(e);head.append(eventName,days);item.append(head);
        if(rows.length){
...
async function loadHistory(){if(loaded)return;const [{data,error},{data:attendees,error:attendeesError},{data:results,error:resultsError}]=await Promise.all([db.rpc('v2_get_public_history'),db.rpc('v2_get_public_history_attendees'),db.rpc('v2_get_public_history_results')]);if(error){toast('Storico non disponibile: '+error.message);return}historyData=data||historyData;if(!attendeesError&&Array.isArray(attendees)){const byTitle=new Map(attendees.map(row=>[row.event_title,Array.isArray(row.athletes)?row.athletes:[]]));historyData.events=(historyData.events||[]).map(event=>({...event,confirmed_athletes:byTitle.get(event.title)||event.confirmed_athletes||[]}))}historyResults=new Map();if(!resultsError&&Array.isArray(results))results.forEach(row=>historyResults.set(resultKey(row.athlete_name,row.event_title),Array.isArray(row.results)?row.results:[]));loaded=true;renderAll()}
